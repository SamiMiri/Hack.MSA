...
import {
  Alert, Image, KeyboardAvoidingView, Modal, Platform, Pressable, SafeAreaView, ScrollView,
...
function SplashScreen({ colors, onPlay, profile }: any) {
  return (
    <View style={{ flex: 1, backgroundColor: colors.background, alignItems: "center", justifyContent: "center" }}>
      {profile?.username && (
        <View style={{ position: "absolute", top: 56, alignItems: "center", flexDirection: "row", gap: 10 }}>
          <AvatarDisplay colors={colors} avatarId={profile.avatarId} avatarUri={profile.avatarUri} name={profile.username} size={36} />
          <Text style={{ color: colors.mutedForeground, fontFamily: "Inter_500Medium", fontSize: 13 }}>
            Signed in as <Text style={{ color: colors.foreground, fontFamily: "Inter_700Bold" }}>{profile.username}</Text>
          </Text>
        </View>
      )}
      {/* Curved title with a downward arc pointing toward the play button */}
      <View style={{ alignItems: "center", marginTop: -80, marginBottom: 28 }}>
        <View style={{ flexDirection: "row", alignItems: "flex-end" }}>
          <Text style={{ fontSize: 52, fontFamily: "Inter_800ExtraBold", color: colors.foreground, letterSpacing: -1 }}>Next</Text>
          <View style={{ width: 8 }} />
          <Text style={{ fontSize: 52, fontFamily: "Inter_800ExtraBold", color: colors.primary, letterSpacing: -1 }}>Step</Text>
        </View>
        {/* Curved underline */}
        <View style={{
          width: 210, height: 44, marginTop: -6,
        ...
        {/* Small arrow tip pointing down to the play button */}
        <View style={{ marginTop: 6 }}>
          <Feather name="chevron-down" size={22} color={colors.primary} />
        </View>
      </View>

      {/* Big centered play button, slightly below mid */}
      <Pressable onPress={onPlay} style={({ pressed }) => ({
        width: 150, height: 150, borderRadius: 80,
        backgroundColor: colors.primary,
        alignItems: "center", justifyContent: "center",
        shadowColor: colors.primary, shadowOpacity: 0.45, shadowRadius: 24, shadowOffset: { width: 0, height: 10 },
        elevation: 12,
        transform: [{ scale: pressed ? 0.95 : 1 }],
        marginTop: 10,
      })}>
        <Feather name="play" size={64} color="white" style={{ marginLeft: 8 }} />
      </Pressable>

      <Text style={{
        position: "absolute", bottom: 40,
        color: colors.mutedForeground, fontFamily: "Inter_500Medium", fontSize: 13,
      }}>
